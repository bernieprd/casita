...
import { useCreateFinanceIncome, useUpdateFinanceIncome, useDeleteFinanceIncome, eurosToCents, centsToEuros } from '@/api/finance'

const INCOME_TAGS = ['salary', 'food_allowance', 'freelance', 'investment', 'rental', 'gift', 'other']

interface Props {
  periodId: string
  entry?: FinanceIncome | null
  open: boolean
  onClose: () => void
}

export default function IncomeForm({ periodId, entry, open, onClose }: Props) {
  ...
  const { mutate: create, isPending: creating } = useCreateFinanceIncome()
  const { mutate: update, isPending: updating } = useUpdateFinanceIncome()
  const { mutate: remove, isPending: deleting } = useDeleteFinanceIncome()

  function handleSave() {
    const amountCents = eurosToCents(amount)
    if (!source.trim() || isNaN(amountCents) || amountCents < 0) return

    const payload = { source: source.trim(), tag: tag === 'none' ? null : tag, amountCents }
    ...
    <Sheet open={open} onOpenChange={v => { if (!v) onClose() }}>
      <SheetContent side="bottom" className="rounded-t-2xl">
        <SheetHeader className="mb-4">
          <SheetTitle>{isEdit ? t('finance.editIncome') : t('finance.addIncome')}</SheetTitle>
        </SheetHeader>
          ...
          <div className="space-y-1.5">
            ...
            <Select value={tag} onValueChange={setTag}>
              ...
              <SelectContent>
                ...
                {INCOME_TAGS.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
